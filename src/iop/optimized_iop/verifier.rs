...
// use crate::fri::*;
use super::{fri::*, tree_hasher::TreeHasher};
use super::{IOP, tree::IopTree, query::IopQuery};
...
pub struct InstanceProof<F: PrimeField, T: Transcript<F>, I: IOP<F>, P: FriProofPrototype<F, I>, PR: FriProof<F, I>, FRI: FriIop<F, IopType = I, ProofPrototype = P, Proof = PR>, A: ARPType> {
    pub f_at_z_m: Vec<F>, 
    // pub f_iop_roots: Vec<<<<I::Tree as IopTree<F> >::Hasher as TreeHasher<F>>::InnerIopTreeHasher as IopTreeHasher<F>>::HashOutput>,
    pub f_iop_root: <<<I::Tree as IopTree<F> >::Hasher as TreeHasher<F>>::InnerIopTreeHasher as IopTreeHasher<F>>::HashOutput,
    pub g_iop_root: <<<I::Tree as IopTree<F> >::Hasher as TreeHasher<F>>::InnerIopTreeHasher as IopTreeHasher<F>>::HashOutput,

    // pub f_queries: Vec<I::Query>,
    pub f_query: I::Query,
    pub g_query: I::Query,

    pub h1_iop_roots: Vec<<<<I::Tree as IopTree<F> >::Hasher as TreeHasher<F>>::InnerIopTreeHasher as IopTreeHasher<F>>::HashOutput>,
    pub h2_iop_roots: Vec<<<<I::Tree as IopTree<F> >::Hasher as TreeHasher<F>>::InnerIopTreeHasher as IopTreeHasher<F>>::HashOutput>,
...
impl<F: PrimeField, T: Transcript<F>, I: IOP<F>, P: FriProofPrototype<F, I>, PR: FriProof<F, I>, FRI: FriIop<F, IopType = I, ProofPrototype = P, Proof = PR>> Verifier<F, T, I, P, PR, FRI, PerRegisterARP> {
    ...
    pub fn verify(
        ...
        // }
        scratch_space.transcript.commit_bytes(proof.f_iop_root.as_ref());
        ...
        //         return Ok(false);
        //     }
        //     f_ldes_at_x.push(query.value()[0]);
        // }

        if !I::verify_query(&proof.f_query, &proof.f_iop_root){
            return Ok(false);
        }
        ...
        let f_ldes_at_x = proof.f_query.value();
        ...
        let g_lde_at_x = proof.g_query.value()[0];
