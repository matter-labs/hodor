...
// use crate::fri::*;
use super::fri::*;
use super::fri::{FriProof, FriProofPrototype};
use super::iop::IOP;
    ...
    > Prover<F, T, I, P, PR, FRI, PerRegisterARP>
{
    ...
    pub fn prove_with_ntt(
        ...
        for w in witness_polys.iter() {
            // let f_lde = w.clone().lde(&self.worker, self.lde_factor)?;
            let f_lde = w.clone().bitreversed_lde_using_bitreversed_ntt(
                &self.worker,
                self.lde_factor,
                bitreversed_omegas,
                &coset_factor,
        ...
        //     self.ali
        //         .calculate_g(&mut transcript, witness_polys.clone(), &self.worker)?;
        let g_bitreversed = self.ali.calculate_g_with_ntt(
            &mut transcript,
            witness_polys.clone(),
            &self.worker,
            bitreversed_omegas,
        ...
        // println!("f bitreversed has size {}", f_ldes[0].as_ref().len());
        // println!("g bitreversed has size {}", g_bitreversed.as_ref().len());
...
fn test_optimized_fib_prover() {
    ...
    let size = witness.clone().expect("")[0].len();
    ...
    let proof = prover
        .prove_with_ntt(witness, &bitreversed_omegas)
        .expect("must work");
...
fn test_multiple_fr_into_buf() {
    ...
    for (idx, el) in values.iter().enumerate() {
        ...
        repr.write_le(&mut buf[idx * 32..(idx + 1) * 32][..])
            .expect("will write");
        println!("el {:?}", repr);
