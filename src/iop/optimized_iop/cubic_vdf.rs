    ...
    fn into_arp(self) -> (Option<Vec<Vec<F>>>, InstanceProperties<F>) {
        ...
        let props = InstanceProperties::<F> {
            ...
            boundary_constraints: vec![
                initial_c0_constraint,
                initial_c1_constraint,
                final_c0_constraint,
                final_c1_constraint,
            ],
        };
...
fn try_prove_cubic_vdf() {
    ...
    let start = Instant::now();
    let f_ldes: Vec<_> = witness_polys
        .iter()
        .map(|w| w.clone().lde(&worker, lde_factor).expect("must work"))
        .collect();
    println!("F LDEs is done after {} ms", start.elapsed().as_millis());

    let start = Instant::now();
    let f_oracles: Vec<_> = f_ldes
        .iter()
        .map(|l| Blake2sIopTree::create(l.as_ref()))
        .collect();
    ...
    let g_poly_interpolant = ali
        .calculate_g(&mut transcript, witness_polys.clone(), &worker)
        .expect("is some");
    ...
    let (h1_lde, h2_lde, _, _) = ali
        ...
        .expect("must work");
    println!(
        "H1 and H2 oracles done after {} ms",
        start.elapsed().as_millis()
    );

    println!(
        "Total proving time w/o FRI is {} ms",
        total_start.elapsed().as_millis()
    );

    let h1_fri_proof = NaiveFriIop::<Fr, TrivialBlake2sIOP<Fr>>::proof_from_lde_by_values(
        &h1_lde, lde_factor, 1, &worker,
    );
    let h2_fri_proof = NaiveFriIop::<Fr, TrivialBlake2sIOP<Fr>>::proof_from_lde_by_values(
        &h2_lde, lde_factor, 1, &worker,
    );

    println!(
        "Total proving time with FRI is {} ms",
        total_start.elapsed().as_millis()
    );
}

#[test]
fn test_optimized_cubic_vdf_high_level_prover() {
    use std::time::Instant;
    // use crate::iop::blake2s_trivial_iop::TrivialBlake2sIOP;
    // use crate::fri::*;
    use super::fri::*;
